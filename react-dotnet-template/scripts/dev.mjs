...
const vite = await createServer({ configFile: join(root, 'frontend/vite.config.ts'),
...
const onStop = () => { void stop().catch(error => { console.error(error); process.exitCode = 1; }); };
process.once('SIGINT', onStop);
process.once('SIGTERM', onStop);
let control;
if (process.env.AIDD_CONTROL_STDIN === '1') {
    control = createInterface({ input: process.stdin });
    control.on('line', line => { if (line === 'shutdown') { control.close(); onStop(); } });
}
try {
    await vite.listen();
    ...
    child = spawn('dotnet', [join(root, 'dist/backend/App.dll')], {
        ...
        env: { ...process.env, HOST: host, PORT: port, DEV_ORIGINS: url,
            ASPNETCORE_ENVIRONMENT: 'Development', AIDD_CONTROL_STDIN: '1' },
    });
    await new Promise((resolve, reject) => {
        ...
        lines.on('line', line => {
            console.log(line);
            if (line.startsWith('AIDD_READY ')) { clearTimeout(timer); resolve(); }
        });
        child.once('error', error => { clearTimeout(timer); reject(error); });
        child.once('exit', (code, signal) => {
            clearTimeout(timer);
            reject(new Error(`.NETが終了しました: ${code ?? signal}`));
            if (!stopping) { process.exitCode = 1; control?.close(); onStop(); }
        });
    });
