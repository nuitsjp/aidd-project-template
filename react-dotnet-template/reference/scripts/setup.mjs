...
const misePath = process.env.PATH.split(delimiter)
    .map(directory => join(directory, miseName)).find(existsSync);
if (!misePath)
    throw new Error('miseがPATHに見つかりません。mise run setup から実行してください。');
const escapedMisePath = realpathSync(misePath).replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;');
writeFileSync(join(root, 'backend/mise.local.props'),
