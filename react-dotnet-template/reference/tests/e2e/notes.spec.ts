...
test('メモを作成・編集して保存する / メモを作成・編集して保存する 作成・編集がUIからSQLiteへ確定する', async ({
  page,
  app,
}) => {
  await signIn(page);
...
test('メモを作成・編集して保存する / 不正な入力の保存を拒否する 不正入力ではDBを更新せず下書きを保持する', async ({
  page,
  app,
}) => {
  await signIn(page);
...
test('メモを作成・編集して保存する / メモを削除する 削除もSQLiteへ確定する', async ({
  page,
  app,
}) => {
  await signIn(page);
