...
if (mode !== 'dev' && mode !== 'hosted')
    throw new Error('E2E_MODEはdevまたはhostedです。');
...
function updateViteProxyTarget(server: ViteDevServer, target: string) {
    ...
    if (!proxy || Array.isArray(proxy))
        throw new Error('Viteのproxy設定を読み取れません。');
...
export const test = base.extend<{
    ...
    app: async ({ serveFrontend }, use, testInfo) => {
        ...
        let viteAddress = '';
        let pid = 0;
        const append = (chunk: Buffer | string) => { output = (output + chunk.toString()).slice(-128 * 1024); };

        async function startVite() {
            if (mode !== 'dev' || !serveFrontend)
                return;
            if (vite) {
            ...
            try {
                if (running.exitCode !== null || running.signalCode !== null) {
                    if (running.exitCode !== 0 || running.signalCode)
                        throw new Error(`E2Eサーバーが異常終了しました code=${running.exitCode ?? 'null'} signal=${running.signalCode ?? 'null'}: ${output}`);
                    return;
                }
                await new Promise<void>((resolveStop, reject) => {
                    ...
                    const finish = (code: number | null, signal: NodeJS.Signals | null) => {
                        ...
                        if (forced)
                            ...
                            reject(new Error(`E2Eサーバーが異常終了しました code=${code ?? 'null'} signal=${signal ?? 'null'}: ${output}`));
                        else
                            resolveStop();
                    };
                    running.once('exit', finish);
                    if (running.stdin && !running.stdin.destroyed)
                        running.stdin.write('shutdown\n');
                    else
        ...
        async function start() {
            ...
            backendAddress = '';
            const running = spawn('dotnet', [resolve(mode === 'dev' ? 'dist/backend/App.dll' : 'dist/server/App.dll')], {
                cwd: process.cwd(),
            ...
            await new Promise<void>((ready, reject) => {
                ...
                const failed = (code: number | null, signal: NodeJS.Signals | null) => {
                    fail(new Error(`サーバー起動失敗 code=${code ?? 'null'} signal=${signal ?? 'null'}: ${output}`));
                };
                const timer = setTimeout(() => fail(new Error(`E2Eサーバーの起動が期限を超えました: ${output}`)), 45000);
                lines.on('line', line => {
                    if (settled || !line.startsWith('AIDD_READY '))
                        return;
                    try {
        ...
        const instance: IsolatedApp = {
            ...
            rows: () => {
                ...
                try {
                    ...
                    return db.prepare('SELECT owner_id,title,body,version FROM notes ORDER BY title').all() as unknown as ReturnType<IsolatedApp['rows']>;
                }
                finally {
                    db.close();
                }
        ...
        try {
            await start();
            testInfo.annotations.push({ type: 'isolated-instance', description: `mode=${mode}; pid=${pid}; worker=${testInfo.workerIndex}; DB=temporary-file` });
            await use(instance);
        }
        finally {
            try {
