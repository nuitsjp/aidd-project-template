...
import {
  ...
  removeNote,
  saveNote,
} from '../../frontend/src/features/notes/access.ts';
...
describe('メモHTTPアクセス', () => {
  ...
  it('一覧はGET /api/notesを呼び出す', async () => {
    const notes = [
      { id: 'note-1', title: '題名', body: '本文', version: 1, updatedAt: '2026-01-01T00:00:00Z' },
    ];
    ...
    expect(fetchMock.mock.calls).toEqual([
      ...
      [
        '/api/notes/remove',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ id: 'note-1', version: 2 }),
        },
      ],
      [
        '/api/notes/preview',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ titles: '一件目', body: '共通本文' }),
        },
      ],
      [
        '/api/notes/import',
  ...
  it('HTTPエラーの公開メッセージを保持する', async () => {
    ...
    await expect(
      saveNote({ title: '題名', body: '本文', id: 'note-1', version: 1 }),
  ...
  it('成功応答の不正JSONを成功値として扱わない', async () => {
    ...
    await expect(requestJson('/api/notes')).rejects.toThrow(
      'サーバーの応答を読み取れませんでした。',
    );
  });
});
