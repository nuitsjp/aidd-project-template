...
try {
  if (process.argv.length !== 3) {
    throw new Error('使い方: mise run init:wails <新しい出力先>');
  }
  const destination = resolve(process.argv[2]);
  const sources = ['template', 'wails-template'].map(name => resolve(root, name));
  for (const source of sources) {
    const path = relative(source, destination);
    if (path === '' || (!isAbsolute(path) && path !== '..' && !path.startsWith(`..${sep}`))) {
      throw new Error('出力先はtemplate/・wails-template/の外に指定してください。');
    }
  }
  ...
  console.log(`Wailsの初期状態を生成しました: ${destination}`);
} catch (error) {
