...
- **テストコード**: 対象系列の段階1〜4ではテストコードを作成・変更せず、手動で動作確認します。段階4の実処理完了後、段階5で合意したシナリオを固定する E2E テストを追加・更新・実行してから次の系列へ進みます。既存テストは維持します（詳細は [モック標準第2節](docs/standards/mock-driven-development.md#workflow)）。
- **画面確認の準備**: 画面での確認を依頼する場合は、事前にアプリケーションを起動し、対象画面の URL を提示します。
- **停止点**: 人の確認を待つ停止点は、全体設計の合意、系列ごとの段階1（記述確認）・段階3（動作合意）・段階5（完了判断）、およびテーブル設計の合意（段階4開始前）です。UI 確認を省略しても、必要なテーブル設計の合意は省略しません。停止点では作業を中断して応答を待ち、得られた原文を合意記録に引用します。質問は未確定事項に絞り、合意済み事項の再確認は行いません（合意待ち中の許容作業は [モック標準第2節](docs/standards/mock-driven-development.md#workflow) 参照）。
