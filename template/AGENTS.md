...
作業の現在地は [PLAN.md](PLAN.md) 第1節が唯一の正本です。着手前に現在のユースケースと段階を確認し、自己判断で段階を進めてはなりません。テンプレート初期の記入欄や例示は確定仕様や検証実績ではないため、前提として扱いません。
...
- **停止点**: 人の確認を待つのは「全体設計の合意」と「ユースケースの動作合意（段階3）」の2箇所です。停止点では作業を中断して応答を待ち、応答の原文を合意記録に引用します。質問は未確定事項に絞り、合意済み事項の再確認は求めません。合意待ち中に可能な作業は [モック標準第2節](docs/standards/mock-driven-development.md#workflow) に従います。
...
- **完了基準と報告**: 完了前に `scripts/doc_check.py` を実行して出力を報告に含め、[完了基準](docs/standards/design-and-documentation.md#completion) に照らして確認します。未実施の検証は「未検証」と明記し、未検証のまま状態語を進めません。エラー発生時は出力を提示します。
