...
作業の現在地は [PLAN.md](PLAN.md) 第1節を正本とします。着手前に現在のユースケース・系列と段階を確認し、自己判断で段階を進めません。初期の記入欄や例示は確定仕様や検証実績ではないため、前提としません。
...
| 系列の追加・変更、次の系列やユースケースへ進む判断 | [適用範囲](docs/document-policy.md#mock-scope)、[モック駆動開発の標準](docs/standards/mock-driven-development.md#workflow)、[PLAN.md](PLAN.md) 第3節の状態語 |
...
- **停止点**: 人の確認を待つのは「全体設計の合意」と、系列ごとの段階1（記述の確認）・段階3（動作合意）・段階5（完了判断）です。停止点では作業を中断して応答を待ち、得られた原文を合意記録に引用します。質問は未確定事項に絞り、合意済み事項の再確認は行いません。合意待ち中の作業は [モック標準第2節](docs/standards/mock-driven-development.md#workflow) に従います。
...
- **完了基準と報告**: 完了前に `scripts/doc_check.py` を実行して出力を報告に含め、[完了基準](docs/standards/design-and-documentation.md#completion) に照らして確認します。未実施の検証は「未検証」と明記し、状態語を進めません。エラー発生時は出力をそのまま提示します。
