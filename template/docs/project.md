...
ユースケースの本文・シナリオ（主成功系列・拡張系列）・合意記録は `usecases/UC-n.md` に集約し、下表から参照します。案の検討は [提示と保存の手順](standards/mock-driven-development.md#discussion) に従い、全文をメッセージで議論して利用者の確認後に保存します（未着手の UC は本文を作成せず、リンクも付けません）。
...
- 本表は最新結果のみを記録し、履歴は累積しません（系列と構成の組ごとに1行）。
- 参照欄にはコミットハッシュまたは CI 実行結果への参照のみを記録し、ログや画像は含めません。
- 未実施の行は合否欄を「未検証」とします（モック構成の合格のみで完了扱いにしません）。
