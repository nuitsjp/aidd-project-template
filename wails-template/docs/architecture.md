...
起動時は設定読込 → Logger・状態管理・機能 Service 生成 → Wails 登録 → ウィンドウ作成の順に行います。終了時は React の未保存確認 → Go の稼働処理確認 → 終了許可 → Service 停止・Logger 解放の順に行い、編集中の画面を直接閉じず、WindowClosing と ShouldQuit を同じ確認経路へ接続します。
...
UC-3 の更新も確認後に実行しますが、NSIS 起動後は別プロセスとなるため、インストーラーの成功を Go の戻り値で表しません。署名・ハッシュ検証失敗や起動失敗では現在のアプリを維持し、適用失敗は NSIS が通知します。
