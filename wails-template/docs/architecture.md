...
DB テーブルや ORM は使用しません。`notes.json`（版番号1、メモ配列、項目: `id`・`title`・`body`・`updatedAt`）を Go で読み込み、一時ファイルへの書き込みと同期を経て置換します（Windows の `MoveFileExW` による置換・完了保証）。読込失敗時に空データで上書き保存しません。
