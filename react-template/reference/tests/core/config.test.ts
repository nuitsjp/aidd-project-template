...
test('proxy構成では認証用公開originを明示する', () => { assert.throws(() => readConfig({ AUTH_MODE: 'proxy' }), /PUBLIC_ORIGIN/); assert.equal(readConfig({ AUTH_MODE: 'proxy', PUBLIC_ORIGIN: 'https://example.com' }).authMode, 'proxy'); });
