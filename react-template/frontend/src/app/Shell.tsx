...
function Application() {
    ...
    if (!session.data.user)
        return <main className={classes.login}>
        <div className={classes.brand}>AIDD / REACT TEMPLATE</div>
        <Title order={1}>対話から、保存まで。</Title>
        <Text c="dimmed">React · tRPC · Node.js · SQLite</Text>
        <div className={classes.card}>
        <Title order={2} size="h3">参照実装を開始する</Title>{session.data.mode === 'demo' ? <Stack mt="md">
            <Alert color="yellow">ローカル参照用のユーザー選択です。認証機能ではありません。</Alert>
            <Group>
            <Button loading={signIn.isPending} onClick={() => signIn.mutate('alice')}>Aliceで開始</Button>
            <Button variant="light" loading={signIn.isPending} onClick={() => signIn.mutate('bob')}>Bobで開始</Button>
            </Group>{signIn.error && <Alert color="red">ユーザーの選択に失敗しました。</Alert>}</Stack> : <Alert color="red" mt="md">認証プロキシから利用者情報が渡されていません。</Alert>}</div>
        </main>;
    ...
    <Text size="xs" mt="sm" c="gray.4">対話はReact、保存はNode.js。<br />サンプルデータはDBへ保存されます。</Text>
    </div>
    </aside>
