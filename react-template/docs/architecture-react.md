...
E2E は `tests/e2e/fixtures.ts` を利用し、1テストごとに独立した環境を割り当てます。

- **1テストごとに、1ブラウザContext・1本番Nodeプロセス・OS自動割当ポート・1一時SQLiteファイルを生成**。
- マイグレーションや業務処理は本番と同一コードを実行。
- UI から操作後、画面確認に加え別の読取専用 DB 接続でコミット済みデータを検証。
- 共有 DB の全削除やテスト用リセット API、外側トランザクションは使用しない。
- fixture が成否を問わずサーバー停止・接続解放・一時領域削除を実行。

テスト間分離と同一 DB 競合の検証は区別します。同一 DB の競合検証は、1テスト内で複数 Page/Context を生成して実施します。

## 5. 配備・認証

本番環境は同一版の UI とサーバーを一体で配備し、DB 領域を配布物から分離します。利用者はリクエストごとに解決し、所有者条件を保存・取得へ適用します。

demo ユーザー選択はローカル参照用です。本番共有利用時は認証プロキシと HTTPS を経由させ、検証済みユーザー情報のみを API へ渡します（ID 基盤自体の再実装は行いません）。
