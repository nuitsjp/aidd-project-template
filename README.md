...
- 採用側への影響: 文書構成が変わるため、差し替えではなく第4節の手順で段階的に取り込みます。機能 F はユースケースに読み替え、`PLAN.md` は新しい構成で作り直します。`docs/standards/` を改変している採用側では判定 6 が NG になるので、配布元の本文に戻し、差分を採用記録に移します。アンカー ID は `#features` と `#f1` を廃止し `#usecases` と `#uc-1` を追加しました。配布物の総行数は Markdown 388 行（版3 は 311 行）、スクリプト 393 行です。
