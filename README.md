...
- **テストの追加時点**: 段階1〜5ではテストコードを作成・変更しません。段階4で DB 操作を含む対象系列の実装をすべて完了し、段階5で利用者が完成系を承認した後、段階6で初めて E2E テストを追加・実行します。本番実装の修正が必要になった場合は段階4に戻り、利用者の再承認後に E2E 実装を再開します。
