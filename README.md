...
ユースケースと確認した事実は `docs/project.md`、全体構造と設計判断は `docs/architecture.md` に置きます。これ以外の規約・方針文書は新設しません。`docs/standards/` は配布元からの輸入物として編集せず、差分は `docs/document-policy.md` 第1節に記録します。

`scripts/doc_check.py` の実行には Python 3 が必要です。Python を使わない構成では導入せず、その旨を `docs/document-policy.md` 第1節の差分欄に書きます。本テンプレートは [MIT ライセンス](LICENSE) で配布しているため、コピー時も著作権表示と許諾文を保持してください。
...
コピー後に `python scripts/doc_check.py` を実行し、リンクとアンカーが解決することを確認します。コピー後の文書はプロジェクト内で完結して参照するため、配布元リポジトリとの同期設定は不要です。
...
以下の順序で、最初のユースケース 1 件を実処理まで通します。未着手ユースケースの画面や系列を先に揃える必要はありません。
...
### 補足事項
- **モック対象外の変更**: 段階2・3を省略し、段階4・5（実装と検証）を行います。文書のみの変更は `scripts/doc_check.py` の実行で整合を確認します。
- **記入欄 `{{...}}` の扱い**: 初期段階ですべて埋める必要はありません。未定事項は推測で埋めず `PLAN.md` で管理します。
- **反復開発**: UC-1 が完了した時点でセットアップは完了です。以降はユースケース単位で順序5を反復します。
...
- `scripts/doc_check.py` の出力に NG がないこと。
...
- 採用側への影響: 文書構成が変わるため、差し替えではなく第4節の手順で段階的に取り込みます。機能 F はユースケースに読み替え、`PLAN.md` は新しい構成で作り直します。`docs/standards/` を改変している採用側では判定 6 が NG になるので、配布元の本文に戻し、差分を採用記録に移します。アンカー ID は `#features` と `#f1` を廃止し `#usecases` と `#uc-1` を追加しました。配布物の総行数は Markdown 388 行（版3 は 311 行）、スクリプト 383 行です。
