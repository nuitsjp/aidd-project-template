...
| 2 | `README.md`（概要）、`docs/project.md` 第1・2節（目的、対象、制約、受け入れ条件）、第3節のカタログ表（UC ID、主アクター、目的、実装順序）の案をメッセージで議論し、利用者の確認後に記入 | 解決する問題と対象外を説明でき、UC-1 が決まっている |
...
| 5 | UC-1 を [モック標準第2節](template/docs/standards/mock-driven-development.md#workflow) の段階1〜5で系列ごとに通す（段階1は案の全文をメッセージで議論し、確認後に `docs/usecases/UC-1.md` へ保存。主成功系列 UC-1-M から着手し、拡張は1本ずつ追加。パターン本体も確認後に記述し、テーブルの追加・変更があれば段階4開始前に設計を合意） | 記述した全系列で `docs/project.md` 第6節の合否表が更新され、UC-1 が完了 |
| 6 | 次のユースケースへ順序5を繰り返す（`docs/architecture.md` は構造やテーブル設計の変更時に再訪） | 進行中の系列が常に1本以下 |

### 補足事項
- **テストの追加時点**: シナリオごとに段階4まではテストコードを作成・変更せず、手動で動作確認します。実処理の実装後、段階5で合意した振る舞いを固定する E2E テストを追加・実行し、完了確認を得てから次のシナリオへ進みます。全ユースケースの実装完了まで待ちません。
- **UI 確認が不要な系列・変更**: 段階2・3を省略し、段階1の確認後、必要なテーブル設計の合意を得てから段階4・5（実装と検証）を実施します。文書のみの変更は `scripts/doc_check.py` で整合性を確認します。
- **テーブル設計の合意**: 対象系列で追加・変更するテーブルのER図・定義・変更点を `docs/architecture.md` 第5節に記述し、提示コミットと利用者の応答を含む合意記録を残します。設計に依存する保存処理やマイグレーションは合意後に実装します。設計変更がなければ再合意は不要です。
...
- テーブルの追加・変更がある場合、設計に依存する実装の前に `docs/architecture.md` 第5節で合意していること（合意の要否・内容は人が確認し、`scripts/doc_check.py` では判定しません）。
