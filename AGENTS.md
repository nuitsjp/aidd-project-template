...
| 対象 | 管理ルール |
| --- | --- |
| `AGENTS.md`、`docs/standards/`、`scripts/doc_check.py` | `template/` だけで管理し、拡張側に上書き用ファイルを置かない。技術固有の注意は既存の固有文書に記載する。 |
| `LICENSE` | ルートだけで管理し、生成時に配置する。 |
| `README.md`、`docs/project.md`、`docs/document-policy.md`、`docs/architecture.md`、`docs/usecases/` | 各拡張側で個別管理する。共通版の記入欄で固有の現在の仕様・制約・検証手順を置き換えない。 |
